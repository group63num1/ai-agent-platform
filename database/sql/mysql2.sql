...
('deepseek-ai/DeepSeek-R1-Distill-Qwen-7B', 'deepseek-r1', 'sk-24c630328e3d478aa7a8156ac1ab6dca', 'https://dashscope.aliyuncs.com/compatible-mode/v1', 'deepseek-r1', 16000, 0.5, 0.9, 50, 0, 0, 1, 30, 3, 2, 1, 'DeepSeek-R1 在后训练阶段大规模使用了强化学习技术，在仅有极少标注数据的情况下，极大提升了模型推理能力。在数学、代码、自然语言推理等任务上，性能较高，能力较强。'),
...
('14_controlDevice', 'controlDevice', '控制设备 - 控制LED、继电器、舵机、PWM等设备', '1.2.0', 'POST https://plugin.aiot.hello1023.com/plugin/control', '\"  - device_uuid (body): 设备UUID，必填: True\\n  - port_type (body): 设备类型：led(LED灯)、relay(继电器)、servo(舵机)、pwm(PWM输出)，必填: True, 枚举值: [\'led\', \'relay\', \'servo\', \'pwm\']\\n  - port_id (body): 端口ID：LED和继电器为1-4，舵机为1-4，PWM为1-2，必填: True\\n  - action (body): 动作：on(打开)/off(关闭)/set(设置值，用于舵机角度或PWM占空比)，必填: True, 枚举值: [\'on\', \'off\', \'set\']\\n  - value (body): 设置值：舵机角度(0-180)或PWM占空比(0-100)，仅当action为set时需要，必填: False\"', '{}', '2025-12-15T23:55:30.888529', '2025-12-15T23:55:30.888543'),
('14_executePreset', 'executePreset', '执行预设 - 通过preset_key执行用户自定义预设', '1.2.0', 'POST https://plugin.aiot.hello1023.com/plugin/preset', '\"  - device_uuid (body): 设备UUID，必填: True\\n  - preset_name (body): 预设标识(preset_key)，如：led_blink_k8x9y2，必填: True\\n  - parameters (body): 可选参数（通常为空），必填: False\"', '{}', '2025-12-15T23:55:30.893076', '2025-12-15T23:55:30.893088'),
('14_getSensorData', 'getSensorData', '查询传感器 - 获取各类传感器数据（温度、湿度、雨水、DS18B20等）', '1.2.0', 'GET https://plugin.aiot.hello1023.com/plugin/sensor-data', '\"  - uuid (query): UUID，必填: True\\n  - sensor (query): 传感器类型，必填: True, 枚举值: [\'温度\', \'湿度\', \'雨水\', \'雨水级别\', \'DS18B20\', \'DS18B20温度\', \'temperature\', \'humidity\', \'rain\', \'rain_level\']\"', '{}', '2025-12-15T23:55:30.879539', '2025-12-15T23:55:30.879555'),
