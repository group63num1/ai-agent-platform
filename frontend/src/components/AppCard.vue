...
const handleClick = () => {
  // 如果是智能客服助手，跳转到聊天页面
  if (props.item.id === 1 || props.item.name === '智能客服助手') {
    router.push('/home/chat')
  } else {
    // 其他应用可以在这里添加处理逻辑
    console.log('点击了应用:', props.item.name)
  }
}
</script>
...
<template>
  <article
    class="group rounded-2xl border border-slate-200 bg-white p-5 shadow-[0_1px_0_rgba(0,0,0,0.02)]
           hover:shadow-md hover:-translate-y-0.5 transition-all duration-200 focus-within:shadow-md cursor-pointer"
