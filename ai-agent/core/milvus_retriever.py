...
# e5 模型设置（与知识库建库保持一致）
E5_MODEL_LOCAL = "knowledge/models--intfloat--multilingual-e5-base"
...
class MilvusRetriever:
    ...
    def _init_embeddings(self):
        ...
        try:
            ...
            # 与建库保持一致，使用 e5 多语言模型
            local_path = Path(E5_MODEL_LOCAL)

            if local_path.exists():
                self.embeddings = HuggingFaceEmbeddings(model_name=str(local_path))
                logger.info(f"✅ 嵌入模型加载成功: {local_path}")
            else:
                # 本地未缓存则使用在线模型
                self.embeddings = HuggingFaceEmbeddings(model_name=E5_MODEL_NAME)
                logger.info(f"✅ 已加载在线模型: {E5_MODEL_NAME}")
