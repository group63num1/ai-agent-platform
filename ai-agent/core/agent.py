...
3. 调用时机: 当需要查询专业知识、历史信息或文档内容时使以及你觉得缺少信息时可以使用试试"""
            ...
            # 如果既没有工具也没有RAG，直接生成回答，跳过推理循环
            has_tools = tools_library and tools_library != "当前无可用工具"
            has_rag = rag_library is not None

            if not has_tools and not has_rag:
                logger.info("[chat_stream] 无工具和RAG，直接流式生成回答")
                # 构建简单的提示词
                simple_prompt = f"{default_system_prompt}\n"
                if summary_block or recent_context:
                    simple_prompt += f"【上下文信息】\n"
                    if summary_block:
                        simple_prompt += f"{summary_block}"
                    if recent_context:
                        simple_prompt += f"最近对话：\n{recent_context}\n"
                simple_prompt += f"【当前问题】\n{user_input}\n"
                simple_prompt += "\n请直接回答用户的问题，用自然、流畅的语言表达。\n"

                # 使用流式生成，边生成边透传
                stream_count = 0
                streamed_chunks: list[str] = []
                first_stream_chunk = True
                for delta in self._stream_generate(simple_prompt):
                    if not delta:
                        continue
                    ...
                    if stream_count == 1:
                        yield {"content": delta, "done": False, "summary": summary_to_return}
                    else:
                        yield {"content": delta, "done": False, "summary": None}
                ...
                while reasoning_steps < max_reasoning_steps and not has_sufficient_info:
                    ...
                    # 流式获取推理输出，并在检测到【最终回答】后立即透传给前端
                    buffer: str = ""
                    captured_answer_chunks: list[str] = []
                    started_final_stream: bool = False
                    stream_count: int = 0
                    first_final_chunk: bool = True
                    for delta in self._stream_generate(reasoning_prompt):
                        if not delta:
                            continue
                        buffer += delta

                        if not started_final_stream:
                            idx = buffer.find("【最终回答】")
                            if idx != -1:
                                started_final_stream = True
                                already_streamed_final = True
                                # 提取标记后的现有内容并立即流式输出
                                after = buffer[idx + len("【最终回答】") :]
                                if after:
                                    after = after.lstrip()
                                    if after:
                                        stream_count += 1
                                        captured_answer_chunks.append(after)
                                        first_final_chunk = False
                                        if stream_count == 1:
                                            yield {"content": after, "done": False, "summary": summary_to_return}
                                        else:
                                            yield {"content": after, "done": False, "summary": None}
                                # 后续 delta 直接作为最终回答的延续透传
                                continue
                        else:
                            if first_final_chunk:
                                delta = delta.lstrip()
                                if not delta:
                                    continue
                                first_final_chunk = False
                            stream_count += 1
                            captured_answer_chunks.append(delta)
                            if stream_count == 1:
                                yield {"content": delta, "done": False, "summary": summary_to_return}
                            else:
                                yield {"content": delta, "done": False, "summary": None}

                    if started_final_stream:
                        # 已流式输出最终回答，结束推理循环
                        final_answer = "".join(captured_answer_chunks).strip()
                        has_sufficient_info = True
                        logger.info(
                            f"[推理步骤 {reasoning_steps}] 检测到【最终回答】并已流式透传，长度: {len(final_answer)}"
                        )
                        break

                    # 未出现最终回答标记，按完整推理结果解析决策
                    reasoning_result = buffer.strip()
                    logger.info(
                    ...
                    (
                        ...
                        final_answer,
                        has_sufficient_info,
                    ) = self._parse_unified_reasoning_output(
                        reasoning_result, tools_info
                    )
            ...
            if not final_answer:
                logger.warning(
                    f"[chat_stream] 推理 {reasoning_steps} 步后未生成答案，强制流式兜底回答"
                )
                # 构建兜底提示词，让 LLM 直接回答（流式）
                ...
                first_stream_chunk = True
                try:
                    for delta in self._stream_generate(fallback_prompt):
                        if not delta:
                            continue
                        if first_stream_chunk:
                            delta = delta.lstrip()
                    ...
                    final_answer = "".join(streamed_chunks).strip()
                    logger.info(
                        f"[chat_stream] 兜底流式生成成功，长度: {len(final_answer)}"
                    )
                except Exception as e:
                    logger.error(f"[chat_stream] 兜底流式生成失败: {e}")
                    final_answer = "抱歉，我在处理您的问题时遇到了一些困难。请您换个方式描述问题，或者稍后再试。"

            # 生成最终回答
            if not already_streamed_final:
                logger.info(
                    f"[chat_stream] 准备返回最终回答，内容长度: {len(final_answer)}"
                )
                full_answer = final_answer.lstrip()
                first_chunk = True
                stream_count = 0
                ...
                # 已在推理阶段完成最终回答的流式透传，只需记录历史
                logger.info(
                    f"[chat_stream] 最终回答已在推理阶段透传，内容长度: {len(final_answer)}"
                )
...
"""
    ...
    def _build_unified_reasoning_prompt(
        ...
        if (tools_library and tools_library != "当前无可用工具") or rag_library:
            ...
            prompt += "- **重要**: 若判断不需要调用工具/知识库，必须直接输出【最终回答】，禁止返回【继续推理】或其他无标记的内容。\n"
        # 输出格式
        if tools_library and tools_library != "当前无可用工具":
            prompt += '\n【输出格式】\n- 单次调用：\n{\n  "tool_call": {\n    "tool_name": "工具名称",\n    "method": "GET/POST",\n    "url": "https://api.example.com/endpoint",\n    "parameters": {"param1": "value1", "param2": "value2"}\n  }\n}\n'
            prompt += '\n- 多次调用（按顺序依次执行，同一插件多次也用此格式）：\n{\n  "tool_calls": [\n    {"tool_name": "...", "method": "POST", "url": "...", "parameters": {...}},\n    {"tool_name": "...", "method": "POST", "url": "...", "parameters": {...}}\n  ]\n}\n'
            prompt += "- 若必填参数信息不足（缺少必填字段），直接输出【最终回答】向用户说明并请求补充，格式如下：\n【最终回答】\n[向用户说明缺少哪些字段并请求补充]\n"
        if rag_library:
            prompt += '\n知识库检索请输出如下JSON：\n{"rag_query": "你的检索问题"}\n'
        prompt += "\n**如果不需要调用任何工具或知识库**，直接输出格式如下：\n\n【最终回答】\n[这里输出完整的用自然语言的回答]\n"
    ...
    def _parse_unified_reasoning_output(
        ...
        # 如果没有明确标记，返回CONTINUE_REASONING
        # （推理循环会在达到max_steps时强制生成兜底回答）
