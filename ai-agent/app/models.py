...
class KnowledgeBaseUpdateRequest(BaseModel):
    ...
    delete_files: Optional[List[str]] = Field(None, description="要删除的文件名列表")
...
class KnowledgeBaseQueryRequest(BaseModel):
    ...
    similarity_threshold: Optional[float] = Field(
        None, description="相似度阈值（0.5-1）, 默认 0.7"
    )
    limit: Optional[int] = Field(None, description="返回数量限制，默认20，范围1-50")
